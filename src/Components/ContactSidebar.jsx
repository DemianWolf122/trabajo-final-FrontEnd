...
export default function ContactSidebar({ user }) {
    const { contacts } = useContext(ContactsContext)

    return (
        <div className="sidebar-wrapper">
            {/* HEADER REAL DEL SIDEBAR */}
            <header className="sidebar-header">
                <img src={`https://ui-avatars.com/api/?name=${user}&background=00a884&color=fff`} alt="Tú" className="my-avatar" />
                <div className="sidebar-header-icons">
                    {/* Icono Comunidades */}
                    <button className="icon-btn" title="Comunidades" onClick={() => alert('Sección Comunidades próximamente')}>
                    ...
                    {/* Icono Nuevo Chat */}
                    <button className="icon-btn" title="Nuevo chat" onClick={() => alert('Crear nuevo chat (Próximamente)')}>
                    ...
                    <Link to={`/contact/${contact.id}`} key={contact.id} className="contact-link">
                        ...
                        <div className="contact-item-info">
                            <div className="contact-item-header">
                                <h3>{contact.name}</h3>
                                <span className="contact-date">12:00</span>
                            </div>
                            <div className="contact-item-preview">
                                <span>{contact.messages[contact.messages.length - 1].text.substring(0, 35)}...</span>
                            </div>
                        </div>
                    </Link>
                ))}
